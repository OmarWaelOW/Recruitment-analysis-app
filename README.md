...
On first launch the app creates `.streamlit/auth.toml` and a local `egy_crete.db` SQLite database. The initial login is `admin` / `change-me-now`; change the generated password hash configuration before sharing the app. Candidate and outcome records are stored in SQLite and include creation/update timestamps, so they survive refreshes and restarts.
...
- Excel scorecards can be applied from the sidebar when sheets contain `Criterion` and `Weight` columns; those criteria replace the built-in role criteria for the current run
